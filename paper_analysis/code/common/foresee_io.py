...
FORESEE_DIR = os.environ.get("FORESEE_DIR", r"e:/FORESEE_App/foresee")
...
def list_foresee_files() -> list[str]:
    """List NetCDF files for the current pipeline run.

    Respects ``ENSEMBLE_MEMBER`` env var: when set, includes only the
    observed HUCLIM files plus RCP files whose basename starts with the
    member prefix (e.g. ``CNRM_CCLM_...``). Unset = legacy flat-dir
    behaviour (single-member archive under FORESEE_DIR).
    """
    files = sorted(glob.glob(os.path.join(FORESEE_DIR, "*.nc")))
    member = os.environ.get("ENSEMBLE_MEMBER", "").strip()
    if not member:
        return files
    mem_lower = member.lower()
    kept = []
    for f in files:
        b = os.path.basename(f).lower()
        if "observed" in b:
            kept.append(f)
        elif b.startswith(mem_lower + "_"):
            kept.append(f)
    return kept
