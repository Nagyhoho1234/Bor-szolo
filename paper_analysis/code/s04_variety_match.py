...
_HUGLIN_VARIANT = _os.environ.get("HUGLIN_VARIANT", "").upper()
...
# Variant suffix on the suitability output so V0/V1/V2/V3 coexist side by side.
OUT_DIR = (_CURATED_ROOT / "variety_match" / _HUGLIN_VARIANT) if _HUGLIN_VARIANT else (_CURATED_ROOT / "variety_match")
...
def huglin_trapezoid(hi: float, env: pd.Series) -> float:
    """Huglin sub-score with pluggable plateau shape.

    Controlled by env var ``HUGLIN_VARIANT``:

    - unset or ``V0`` (default): classic flat plateau = 1.0 across [olo, ohi]
    - ``V1``: cos² taper 0.85 -> 1.00 -> 0.85 (15% centrality reward)
    - ``V2``: cos² taper 0.90 -> 1.00 -> 0.90 (10% centrality reward)
    - ``V3``: Gaussian taper 0.87 -> 1.00 -> 0.87 (smoother shape)
    """
    ...
    import os as _os
    variant = _os.environ.get("HUGLIN_VARIANT", "V0").upper()
    ...
    # All V1/V2/V3 variants use a centred plateau. Parameters:
    if variant == "V1":
        edge = 0.85
    elif variant == "V2":
        edge = 0.90
        ...
        raise ValueError(f"unknown HUGLIN_VARIANT: {variant}")

    # Outer ramps rise/fall 0 <-> edge, so continuity at olo/ohi is exact.
    if hi < olo:
        return float(edge * (hi - hmin) / max(olo - hmin, 1e-9))
    if hi > ohi:
        return float(edge * (hmax - hi) / max(hmax - ohi, 1e-9))

    # Inside plateau
    mid = 0.5 * (olo + ohi)
    width = max(ohi - olo, 1e-9)
    if variant in ("V1", "V2"):
        # cos² peak: equals edge at olo/ohi and 1.0 at mid
        import math
        c = math.cos(math.pi * (hi - mid) / width)
        return float(edge + (1.0 - edge) * c * c)
    # V3 Gaussian: sigma chosen so value at plateau edge == edge
    # exp(-((ohi-mid)/sigma)²/2) = (edge - 0) / (1.0 - 0), solving for sigma
    # such that gaussian(edge of plateau) = edge => sigma = (width/2) / sqrt(-2 ln edge)
    import math
    half = width / 2.0
    sigma = half / math.sqrt(-2.0 * math.log(edge))
    g = math.exp(-((hi - mid) ** 2) / (2.0 * sigma * sigma))
    return float(g)
