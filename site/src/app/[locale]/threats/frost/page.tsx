...
export async function generateMetadata({
  ...
  const title = isHu
    ? 'Tavaszi fagykár a magyar borvidékeken'
    : 'Spring frost risk in Hungarian wine districts';
  const description = isHu
    ? 'Hol fenyeget leginkább a fakadás utáni tavaszi fagy a 22 magyar borvidéken — ma és 2100-ban.'
    : 'Which of the 22 Hungarian wine districts face the worst post-budbreak frost risk — today and in 2100.';
  return {
    title,
    description,
    openGraph: {
...
export default async function FrostPage({
  ...
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'threats.frost' });
  ...
  // CCKP doesn't ship "spring frost days" specifically; use minimum-temperature
  // mean (`tasmin`) as a proxy reference. Lower tasmin = higher frost risk.
  const cckp245 = readJson<CckpRow[]>(
    path.join(dataDir, 'cckp', 'cckp_cmip6_tasmin_ssp245_per_district.json'),
  );
  const cckp585 = readJson<CckpRow[]>(
    path.join(dataDir, 'cckp', 'cckp_cmip6_tasmin_ssp585_per_district.json'),
  );

  const INDEX = 'spring_frost_days';
  ...
  return (
    <article className="space-y-6">
      <header>
        <p className="text-sm uppercase tracking-wider text-blue-700">
          {t('kicker')}
        </p>
